...
async def get_current_user(db: AsyncSession = Depends(database), token: str = Depends(oauth2_scheme)) -> UserM:
    ...
    async with db as session:
        query = select(UserM).where(UserM.email == token_data.username)
...
async def get_current_active_user(current_user: UserM = Depends(get_current_user)) -> UserM:
...
async def login_for_access_token(
    ...
    async with db as session:
        query = select(UserM).where(UserM.email == form_data.username)
        result = await session.execute(query)
        user = result.scalar_one_or_none()
