...
async def turlarni_korish(db:AsyncSession = Depends(database)):
    result = await db.execute(select(CategoryM))
    return result.scalars().all()

    
@category_router.post('/post_types')
async def turlar_qoshish(form : CategoryS, db:AsyncSession = Depends(database), current_user : UserM = Depends(get_current_user)):
    try:
...
async def turlarni_tahrirlash(ident:int, form:CategoryS, db:AsyncSession = Depends(database), current_user : UserM = Depends(get_current_user)):
    try:
...
async def ozini_ochirish(ident:int, db:AsyncSession = Depends(database), current_user : UserM = Depends(get_current_user)):
    try:
